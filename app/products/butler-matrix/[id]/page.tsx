...
export default function ButlerMatrixModelPage() {
  const params = useParams();
  const id = params.id as string;
  const model = butlerModelMap[id];
  if (!model) notFound();

  const others = butlerModels.filter((m) => m.id !== id);

  const lossSpec = getSpec(model.specifications, "Insertion loss");
  const portSpec = getSpec(model.specifications, "Number of RF Ports");
  const phaseSpec = getSpec(model.specifications, "Phase Balance");
  const isoSpec = getSpec(model.specifications, "Isolation between ports");
  const dimSpec = getSpec(model.specifications, "Dimensions");

  const heroSpecs = [
    { label: "Frequency Range", value: model.frequencyText },
    { label: "RF Ports", value: portSpec?.typ?.match(/\d+/)?.[0] ?? "—" },
    {
      label: "Insertion Loss",
      value: lossSpec?.typ ? `${lossSpec.typ} dB` : "—",
    },
    {
      label: "Phase Balance",
      value: phaseSpec?.typ ? `${phaseSpec.typ}°` : "—",
    },
    {
      label: "Port Isolation",
      value: isoSpec?.min ? `${isoSpec.min} dB` : "—",
    },
    { label: "Dimensions", value: dimSpec?.typ ? `${dimSpec.typ} mm` : "—" },
  ];

  const sizeLabel = model.matrix.split(" ")[0].replace("x", "×");
  ...
  return (
    <div className="min-h-screen bg-[#f7f7f5] text-zinc-900">
            ...
            <div className="mb-16 grid gap-12 md:grid-cols-12">
              ...
              >
                ...
                <p className="mt-6 leading-relaxed text-zinc-600">
                  The {model.model} delivers repeatable, high-fidelity RF
                  performance through carefully optimized passive network
                  design.
                </p>
      ...
      <section className="relative overflow-hidden bg-[#172556] text-white">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-12 px-6 py-20 md:grid-cols-12 md:px-10 md:py-28">
          ...
          >
            ...
            >
              The {model.model} is ideal wherever repeatable spatial signal
              routing is required without calibration overhead.
            </motion.p>
            <motion.div
              variants={fadeUp}
              custom={3}
          ...
          <div className="mb-12 flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
            <motion.div variants={fadeUp} custom={0}>
              ...
              <p className="mt-4 max-w-xl text-lg leading-relaxed text-zinc-600">
                Complete performance data for the {model.model} passive network.
              </p>
            </motion.div>
          </div>
            ...
            <motion.div variants={fadeUp} custom={3} className="lg:col-span-4">
              <div className="sticky top-28 space-y-4">
                <div className="rounded-2xl border border-zinc-200/80 bg-gradient-to-b from-zinc-100/80 to-zinc-50/80 p-7 shadow-[0_1px_0_0_rgba(0,0,0,0.03)]">
                  ...
                  <p className="mt-2 text-sm leading-relaxed text-zinc-600">
                    Full datasheet with performance graphs, phase diagrams, and
                    mechanical drawings.
                  </p>
                  <a
                    href="#"
                ...
                <div className="rounded-2xl border border-[#172556]/10 bg-[#172556]/[0.04] p-7">
                  ...
                  <p className="mt-2 text-sm leading-relaxed text-zinc-600">
                    We design custom Butler Matrix configurations for
                    specialised frequency ranges and port dimensions.
                  </p>
                  <Link
                    href="/support"
      ...
      {/* ── Functional Diagram Placeholder ── */}
          ...
          >
            <motion.div variants={fadeUp} custom={0} className="max-w-2xl">
              ...
              <p className="mt-6 text-lg leading-relaxed text-zinc-600">
                The {sizeLabel} passive matrix enables simultaneous transmission
                over multiple spatial paths with strictly defined phase
                differentials.
              </p>
            </motion.div>
            ...
            >
              <div className="relative overflow-hidden rounded-[2rem] border border-zinc-200/80 bg-white p-3 shadow-[0_10px_40px_-15px_rgba(0,0,0,0.05)]">
                <div className="flex aspect-[16/9] md:aspect-[21/9] w-full items-center justify-center rounded-2xl border border-zinc-100 bg-zinc-50/50">
                  <div className="flex flex-col items-center gap-4 text-zinc-400">
                    <LayoutDashboard className="size-10 opacity-50" />
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {others.map(
                (
                  ...
                  const mLoss = getSpec(m.specifications, "Insertion loss");
                  const mPhase = getSpec(m.specifications, "Phase Balance");
                  const mPorts = getSpec(
                    m.specifications,
                    "Number of RF Ports",
                  );
                  ...
                  const sizeGradients: Record<string, string> = {
                    ...
                    "4x4": "bg-gradient-to-br from-blue-500/10 to-transparent",
                  };
